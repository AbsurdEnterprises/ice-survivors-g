...
func _ready() -> void:
	...
	if get_tree().has_group("hud"):
		...
		hud.call_deferred("update_hp", current_hp, max_hp)
		hud.call_deferred("update_xp", current_xp, get_xp_required(current_level))
		hud.call_deferred("update_level", current_level)
		
	# Give starting weapon
	add_or_upgrade_item("weapon_01", "weapon")
